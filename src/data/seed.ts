...
async function seedGebruikers(aantal: number) {
    ...
    for(let i = 0; i < aantal; i++){
        ...
        gebruikers.push({
            adres_id: Number(bestaandeAdressen[Math.floor(Math.random() * bestaandeAdressen.length)].adres_id),
            naam: String(naam),
...
async function seedSites(aantal: number) {
    ...
    for(let i = 0; i < aantal; i ++){
        sites.push({
            ...
            verantwoordelijke_id: Number(bestaandeGebruikers[Math.floor(Math.random() * bestaandeGebruikers.length)].gebruiker_id),
        })
    }
    return sites;
}
...
async function seedMachines(aantal: number) {
    ...
    for(let i = 0; i < aantal; i ++){
        machines.push({
            site_id: Number(bestaandeSites[Math.floor(Math.random() * bestaandeSites.length)].site_id),
            product_id: Number(bestaansdeProducten[Math.floor(Math.random() * bestaansdeProducten.length)].product_id),
            technieker_gebruiker_id: Number(bestaandeGebruikers[Math.floor(Math.random() * bestaandeGebruikers.length)].gebruiker_id),
            ...
            productie_status: Object.values(Productie_Status)[Math.floor(Math.random() * Object.values(Productie_Status).length)]
        })
    }
    return machines;
}
...
async function seedOnderhouden(aantal: number) {
    ...
    for(let i = 0; i < aantal; i ++){
        onderhouden.push({
            machine_id: Number(bestaandeMachines[Math.floor(Math.random() * bestaandeMachines.length)].machine_id),
            technieker_gebruiker_id: Number(bestaandeGebruikers[Math.floor(Math.random() * bestaandeGebruikers.length)].gebruiker_id),
